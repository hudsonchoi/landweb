...
select c.id, c.course_code, c.year_no, c.semester_no, c.course_name, c.teacher_name, c.start_date, c.start_time,
       c.day_of_week,
       ...
       case when mc.member_id is not null then 'Y' else 'N' end as requestyn, mc.create_date as requestdate
  from courses c
  left outer join member_course mc on mc.member_id = @MemberId and mc.course_id = c.id and mc.delete_date is null
...
update mc
 ...
 where mc.member_id = @MemberId
   and ',' + @CheckedCourses + ',' not like '%,' + convert(varchar, mc.course_id) + ',%'
...
update mc
  ...
  from member_course mc
 inner join members m on m.id = mc.member_id
 where mc.member_id = @MemberId and mc.course_id = @CourseId and mc.delete_date is not null;
insert into member_course (member_id, course_id, create_date, create_by, update_date, update_by)
select @MemberId, @CourseId,
  ...
  from members m
 where m.id = @MemberId
   and not exists (select * from member_course where member_id = @MemberId and course_id = @CourseId)";
                    SqlCommand cmd = new SqlCommand(strSql, cnn, stCNN);
                    cmd.Parameters.Add("@MemberId", SqlDbType.Int).Value = nMemberId;
